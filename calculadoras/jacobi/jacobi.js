  ...
  calcular() {
    try {
      ...
      if (!this.verificarDominancDiagonal()) {
        Notificaciones.error('Matriz no es diagonalmente dominante. Jacobi puede no converger.');
        return;
      }

      Notificaciones.calcular('Resolviendo con método Jacobi...');
      ...
      for (let k = 0; k < this.estado.maxIter; k++) {
        let xNueva = Array(n).fill(0);
        
        for (let i = 0; i < n; i++) {
          let suma = 0;
          for (let j = 0; j < n; j++) {
            if (i !== j) {
              suma += this.estado.matrizAumentada[i][j] * xAnterior[j];
            }
          }
