const App = {
  ...
  verificarH() {
    ...
    if (Math.abs(hCalculado - h) > 1e-10) {
      advertencia.classList.remove('oculto');
      advertencia.innerHTML = `<p>9 Se ajustará h a ${this.formatear(hCalculado)} para que (xf - x0) sea múltiplo exacto de h.</p>`;
    } else {
      advertencia.classList.add('oculto');
    }
  },
  ...
  calcular() {
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const k4 = this.evaluarFuncion(funcion, xActual + hAjustado, yActual + hAjustado * k3);

        const yNuevo = yActual + (hAjustado / 6) * (k1 + 2 * k2 + 2 * k3 + k4);
      ...
      // Calcular k's para el último punto
      ...
      const k4Final = this.evaluarFuncion(funcion, puntos[n].x + hAjustado, puntos[n].y + hAjustado * k3Final);
      ...
      alert(`Cálculo completado: y(${this.formatear(xf)}) H ${this.formatear(yActual)}`);
    ...
    for (let i = 0; i < numMostrar; i++) {
      ...
      latex += `
        <div class="paso-desarrollo">
          ...
          <p>$$k_4 = f\\left(${this.formatear(actual.x)} + ${this.formatear(this.estado.h)}, ${this.formatear(actual.y)} + ${this.formatear(this.estado.h)} \\cdot ${this.formatear(actual.k3)}\\right) = ${this.formatear(actual.k4)}$$</p>
          <p>$$y_{${i + 1}} = ${this.formatear(actual.y)} + \\frac{${this.formatear(this.estado.h)}}{6}(${this.formatear(actual.k1)} + 2 \\cdot ${this.formatear(actual.k2)} + 2 \\cdot ${this.formatear(actual.k3)} + ${this.formatear(actual.k4)}) = ${this.formatear(siguiente.y)}$$</p>
    ...
    r.puntos.forEach((punto, i) => {
      html += `
        <tr>
          ...
          <td>${this.formatear(punto.k3)}</td>
          <td>${this.formatear(punto.k4)}</td>
        </tr>
      `;
    });
  ...
  graficar() {
    ...
    const layout = {
      title: {
        text: `Método RK4: y' = ${this.estado.funcion}`,
    ...
    Plotly.newPlot('graficoRK4', trazas, layout, config);
  },
  ...
  cargarEjemplo() {
    document.getElementById('selectFuncion').value = '-2*y';
    document.getElementById('inputFuncion').value = '-2*y';
