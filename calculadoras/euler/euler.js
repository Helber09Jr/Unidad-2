const App = {
  ...
  verificarH() {
    ...
    if (!Number.isInteger(n) || n <= 0) {
      ...
      advertencia.innerHTML = `
        <p>  El paso h no divide exactamente el intervalo. Se ajustará automáticamente.</p>
  ...
  calcular() {
    try {
      ...
      alert(`Cálculo completado: ${n + 1} puntos generados. Solución en x = ${this.formatear(xf)}: y H ${this.formatear(yi)}`);
